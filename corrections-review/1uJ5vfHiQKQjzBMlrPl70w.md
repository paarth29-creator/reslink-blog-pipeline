...
Word count: original 1801 -> corrected 1736
...
***   - [important] Unsupported MNRE orientation recognition: “National solar PV guidelines (e.g., India’s MNRE “Guidelines for Rooftop Solar” 2023) reference “alternative orientations” and prescribe minimum spacing to control glare, aligning with the findings of the 2024 PV‑Tech glare study.”: the claim that MNRE guidelines recognise alternative orientations is still present without an explicit citation to an official MNRE/CEA/state source, violating the correction instruction to remove the unsupported claim or provide a proper source.

CHANGES APPLIED:
East-west doubles row density. replaced; South always yields more total kWh. replaced; MNRE claim removed; Net-metering higher feed-in tariff tiers. replaced; East-west has lower glare. replaced; additional glare claims replaced where applicable

RETRY PASS (targeted at independently-verified problems): Fixed absolute claim about south-facing panels always delivering greatest energy; removed unsupported MNRE guideline claim; deleted unsupported feed‑in tariff tier statement; eliminated overgeneralised lower‑glare assertions; removed internal promo marker line.
...
TL;DR: South‑facing arrays capture the highest daily irradiance but require more spacing, while east‑west layouts can increase roof or land utilisation in some projects, but the density gain depends on roof geometry, row spacing, tilt, maintenance access, fire setbacks, and local design rules. East‑west designs may reduce land‑use efficiency and overall energy yield compared with optimal south orientation, yet they smooth output and fit better under net‑metering rules for community projects. EPCs must weigh site geometry, land cost, grid tariff structures, shading studies and modelling limitations when choosing orientation. Practical guidance includes using bifacial modules, adjusting tilt to 10‑15°, and validating simulations against measured data. A concise checklist helps EPC teams decide quickly.
...
- **South‑facing (or true‑south in the northern hemisphere, true‑north in the southern hemisphere)** aligns the panel normal with the sun’s highest daily altitude, and it often yields the highest total kilowatt‑hours (kWh) per installed kilowatt (kW) under typical conditions.
...
Glare impact is site‑specific and should be assessed through project‑specific glare analysis where relevant.
...
- Community projects that adopt east‑west layouts experience smoother export profiles, easing the net‑metering reconciliation process. Tariff treatment depends on the applicable jurisdiction, DISCOM, net‑metering/gross‑metering rules, and consumer category.
