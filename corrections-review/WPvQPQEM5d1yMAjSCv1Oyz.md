...
Word count: original 1747 -> corrected 1894

CHANGES APPLIED:
Stale RPO framing applied; REC automatic earning overclaim applied; CERC fines overbroad applied; Missing current capacity applied; Compliance routes generic not found, skipped

RETRY PASS (targeted at independently-verified problems): Removed stale RPO framing about 3% target by 2022; Added separate compliance routes list (rooftop, open‑access, utility‑scale, captive/group captive, residential subsidy, solar‑park); Deleted leaked promo image marker line
...
- **Solar RPO targets** – Minimum procurement percentages that electricity distributors must source from solar. **India’s solar regulatory environment in 2026 is shaped by renewable purchase/consumption obligations, Green Energy Open Access Rules, ALMM/BIS requirements, state net‑metering or gross‑metering rules, PM Surya Ghar for residential rooftop, and state‑specific approval procedures.**
...
## Distinct Compliance Routes for Solar Projects

Solar projects can satisfy RPO and REC obligations through several separate compliance routes. EPCs should identify the most appropriate route for each project and follow the specific procedural requirements:

- **Rooftop Solar (Commercial & Industrial)** – Eligible under the Grid‑Connected Rooftop Solar Phase‑II extension; requires state‑level net‑metering or gross‑metering approval and REC registration.

- **Open‑Access Solar** – Projects that procure power directly from the grid under the Green Energy Open‑Access Rules, bypassing traditional distribution channels.

- **Utility‑Scale Solar** – Large‑scale power plants (typically >10 MW) that sell electricity to utilities or into the open market; compliance is achieved through REC generation and sale.

- **Captive/Group‑Captive Solar** – Self‑consumption projects for industrial or commercial groups; compliance is met via self‑certified generation and, where required, REC surrender.

- **Residential Subsidy Schemes** – Programs such as PM Surya Ghar that provide subsidies for residential rooftop installations; compliance is demonstrated through subsidy claim documentation and REC registration where applicable.

- **Solar‑Park Development** – Dedicated solar‑park schemes offering land‑allocation incentives and streamlined approvals; compliance is achieved through project‑level REC issuance and adherence to state‑specific RPO targets.
...
Each state publishes its own RPO ceiling, often exceeding the national floor. The MNRE’s RPO tracker lists these targets alongside installed capacity and compliance status. EPCs must align project sizing with the most recent state mandates to avoid penalties.
...
Many states offer additional subsidies for exceeding baseline RPO thresholds. EPCs should consult state energy departments for the latest subsidy schedules.
...
Several states have set targets above the national floor for 2025‑2030. EPCs should prioritize projects in those states to capture higher subsidy rates and mitigate compliance risk for local distributors.
...
Each REC represents 1 MWh of verified solar generation. **REC eligibility, issuance, and trading depend on applicable REC regulations, registration, metering, and compliance conditions;** distributors purchase them to meet RPO obligations.
