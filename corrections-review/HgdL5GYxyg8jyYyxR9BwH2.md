...
Word count: original 1970 -> corrected 2034

CHANGES APPLIED:
CAPEX range added; unit definitions paragraph inserted; 20 % discount rate removed; storage premium answer rewritten

RETRY PASS (targeted at independently-verified problems): Removed visible placeholder [[PROMO_IMAGE_MARKER_DO_NOT_REMOVE_OR_EDIT_THIS_LINE]] occurrences throughout the document.
...
**Unit definitions:**  
- **CAPEX** = total installed cost expressed as **$/MW** (or **$/W**) for the project.  
- **LCOE** = levelized cost of energy expressed as **$/MWh** over the project life.  
- **WACC/discount rate** affects the present‑value calculation of LCOE but is not the same as the EPC’s equipment cost per MW.
...
When paired with battery storage, the overall $/MW rises due to battery capital cost and additional balance‑of‑system components. The impact varies with **battery duration, power rating, chemistry, power conversion system (PCS), grid interconnection requirements, land use, and market conditions**. EPCs should state these assumptions explicitly before quoting any percentage increase.
