...
Word count: original 1970 -> corrected 2097

CHANGES APPLIED:
CAPEX vs LCOE mismatch applied; Unit definitions missing applied; Unsupported 20% discount rate applied (removed); Storage premium too generic applied
...
**Unit definitions:**  
- **CAPEX** = capital expenditure expressed as $/MW (or $/W) for installed capacity.  
- **LCOE** = levelized cost of energy expressed as $/MWh over the project life.  
- **WACC/discount rate** affects the financing component of LCOE but is not the same as EPC’s equipment cost per MW. Keeping these units distinct avoids confusion in cost models.
...
When paired with battery storage, the overall $/MW rises due to several cost components: battery capital cost (which varies with chemistry and duration), power conversion system (PCS) expenses, additional balance‑of‑system equipment, grid interconnection upgrades, land required for siting the storage system, and ongoing operations‑and‑maintenance. The premium depends on the chosen battery technology (e.g., lithium‑ion vs. flow), the required duration (hours of storage), and the scale of the storage system relative to the solar plant. Detailed assumptions about these factors must be stated before quoting any percentage impact.
