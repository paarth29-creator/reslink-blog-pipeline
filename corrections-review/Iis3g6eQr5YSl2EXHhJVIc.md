...
Word count: original 2311 -> corrected 2473

CHANGES APPLIED:
Scope/title mismatch corrected; 500 MW threshold overgeneralisation corrected; Stale India solar target updated; Railway reference re‑framed as past development; Environmental clearance wording revised; MNRE‑approved vendor database wording narrowed
...
TL;DR: Setting up a solar plant in India in 2026 involves two distinct tracks.  For utility‑scale projects that seek support under the MNRE “Development of Solar Parks and Ultra‑Mega Solar Power Projects” scheme, developers must navigate the scheme‑specific requirements (central financial assistance, detailed project reports, and statutory clearances for the park).  For all other solar plants – rooftop, C&I, captive, group‑captive, open‑access or standalone utility projects – the process follows the general approval route that includes land acquisition, DISCOM/STU/CTU grid connectivity, CEIG/CEA approvals, open‑access/PPA negotiations, ALMM/BIS compliance, metering, commissioning, and state‑level approvals.  EPCs should first determine which track applies, then follow the corresponding workflow.  The scheme‑specific track still requires a minimum 500 MW park size (or a justified exception), while the general track has no such size ceiling.  A disciplined workflow that covers site feasibility, approvals, procurement, construction and commissioning ensures timely delivery and eligibility for any applicable incentives.  Reslink’s EPC workflow automates DPR preparation and tracks CFA eligibility, simplifying compliance for scheme‑supported parks.  Following the step‑by‑step guide below helps EPCs move from concept to commercial operation efficiently.
...
The pathway from concept to operational solar plant follows a structured sequence of eligibility verification, documentation, and regulatory approvals. The steps below are organized to address both the MNRE scheme (for solar parks/ultra‑mega projects) and the broader Indian solar approval framework.
...
- Verify ALMM applicability for modules, BIS certification for applicable equipment, inverter/grid‑code compliance, and any DCR or domestic‑content requirement linked to the specific scheme, tender, or subsidy route. *Do not assume that all components must come from an MNRE‑approved vendor database.*
...
Indian Railways has previously announced solar deployment plans on railway land and traction infrastructure, including a 2 GW tender and a pilot at Bina (1.7 MW). This should not be framed as a fresh 2026 tender unless a current official notice is cited.
...
- **✅ Verify eligibility:** Confirm park size, State quota and land status (or follow the general route for non‑park projects).
...
- **✅ Secure clearances:** Environmental, land, transmission and water approvals as applicable to the specific site.
...
Indian Railways has previously announced solar deployment plans on railway land and traction infrastructure, including a 2 GW tender and a pilot project at Bina (1.7 MW). This should not be presented as a fresh 2026 tender unless a current official tender notice is cited.
