...
Word count: original 1061 -> corrected 1111
...
TL;DR: Zero‑export practices aim to keep C &I solar systems from feeding power back to the grid, using inverter firmware and smart‑meter feedback. EPCs need to select compatible inverters, configure dual smart meters, and verify that export is limited in accordance with project‑specific requirements.
...
- **Hybrid inverters** – Solis hybrid models (S5‑EH1P, S6‑EH1P, S6‑EH3P) are presented as examples of implementation; they are supported for dual‑meter setups, though current Solis documentation notes that zero‑export (0 W) is not yet feasible because the inverter does not communicate with an existing PV inverter.
...
Both families may be paired with **Eastron** smart meters that support dual‑address configurations; product mentions are illustrative and not proof of regulatory compliance.

### Certification and documentation steps

1. **Select an inverter** that offers zero‑export capability; product mentions are examples, not proof of regulatory compliance.
...
4. **Record a 24‑hour export test** with the battery fully charged and verify that the export reading on the export‑meter remains within the utility‑specified tolerance and response‑time limits.
...
- Use Eastron meters that support dual‑address configuration (e.g., EM340 firmware 1.12.3 or later) as an example of suitable hardware.
...
- **Configure firmware** – enable the zero‑export mode and lock export limit within the required tolerance where possible.
...
It means the PV‑plus‑storage system is configured so that export power to the grid is limited within the tolerance and response‑time limits required by the utility, meter, inverter, and project approval conditions.
