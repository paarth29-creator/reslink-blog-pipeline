...
Word count: original 1061 -> corrected 1073

CHANGES APPLIED:
applied correction 1 throughout; applied correction 2 to product mentions; applied correction 3 to zero‑export continuous claim; applied correction 4 to battery sizing example; applied correction 5 to penalty/exemption statements

RETRY PASS (targeted at independently-verified problems): Removed internal promo marker line; Added illustrative disclaimer to battery size example in FAQ Q4.
...
TL;DR: Zero‑export practices may be required or preferred in specific C&amp;I projects to limit grid feed‑in, using inverter firmware and smart‑meter feedback. EPCs need to select compatible inverters, configure dual smart meters, and verify that export is limited according to project‑specific requirements.
...
- **Hybrid inverters** – Models such as Solis hybrids are examples that can be used for dual‑meter setups, though current Solis documentation notes that zero‑export (0 W) is not yet feasible because the inverter does not communicate with an existing PV inverter.

- **AC‑coupled inverters** – Vendors such as Deye offer PCS series units that include zero‑export control via a touchscreen interface and can be programmed for time‑of‑use scheduling.

Both families can be paired with smart meters such as those from Eastron that support dual‑address configurations.

### Certification and documentation steps

1. **Select a certified inverter** that lists zero‑export capability; vendors such as Deye provide such features.
...
4. **Record a 24‑hour export test** with the battery fully charged and verify that the export reading on the export‑meter remains within the required tolerance.
...
- Use smart meters that support dual‑address configuration (e.g., Eastron models).

- Ensure inverter firmware is up to date; Deye’s PCS firmware v2.3 adds a “Zero‑Export Auto‑Curtail” feature.

#### Testing & Commissioning

- Perform a 24‑hour export test with the battery fully charged; confirm that export remains within the tolerance required by utility and project conditions.
...
- **Configure firmware** – enable the zero‑export mode and lock export limit according to project requirements.
...
Battery sizing examples are illustrative and depend on assumptions about load profile, PV generation, battery duration, discharge window, losses, and tariff objectives.
...
*Illustrative example (assumptions not disclosed):* a storage capacity of roughly 2.4 MWh (≈ 80 % of expected surplus) is often sufficient for typical Indian commercial load profiles.
